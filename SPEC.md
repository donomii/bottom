...
The default interaction is an event stream suitable for terminals and pipelines. Optional recorders persist human-readable events to text or versioned sessions to JSONL and CSV. Recorded sessions can be queried, replayed, summarized, compared, and visualized.
...
- `bottom compare` to compare process fingerprints, ancestry, counts, failures, and average lifetimes between two JSONL recordings.

Each repeated `-input` supplies a JSONL recording to query, replay, or report. With no input, the command reads `bottom.jsonl`. A command accepts at most 64 explicitly supplied recordings, rejects paths that resolve to the same file, and opens every input read-only.

Each reader validates session envelopes and event schema, kind, time, backend, and session identity as records are reached. Multiple files are merged by event time, sequence, and explicit input order. Filters are applied lazily, so a positive limit can finish before malformed JSON later in a recording.

Reports and comparisons keep exact counts in memory and sort only when rendering output.
...
- `format`: text by default; alternatives are JSONL and CSV.
- `output_path`: empty by default and writes to standard output.
...
Rotation and triggered recording require an output path.
...
- `input_paths`: `bottom.jsonl` by default; `-input` may be repeated up to 64 times to merge distinct recordings.
